...
| 6 | [Model selection, calls, usage, quota, and billing](model-selection-usage-quota-billing.md) | How are main-turn, resume, Agent, Workflow, skill, helper, and fallback models resolved, and how do `/model`, `/effort`, `/fast`, `/usage`, `/usage-credits`, limits, budgets, and billing work? |
| 7 | [Headless streaming and resilience](headless-streaming-and-resilience.md) | How does `-p`/SDK mode validate formats, multiplex frames, prioritize queued prompts, and distinguish interrupt, async-message cancellation, and correlated control cancellation? |
