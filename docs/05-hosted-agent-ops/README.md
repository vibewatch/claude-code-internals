...
| 7 | [Enterprise gateway server](enterprise-gateway.md) | How does `claude gateway --config` join OIDC, Postgres, inference routing, managed settings, spend controls, OTLP relay, and network hardening? |
| 8 | [Media native modules](media-native-modules.md) | Which non-`cli.js` graph modules are embedded, and how are image/audio native helpers loaded? |
| 9 | [Audio capture and voice mode](audio-capture-and-voice.md) | How does `/voice` use local audio capture, recorder fallbacks, a transcription stream, and transcript injection? |
| 10 | [Audio capture native module](audio-capture-native.md) | Which Rust crates (`cpal`/`alsa`), ALSA PCM surface, and N-API exports ship inside `audio-capture.node`? |
| 11 | [Image processor native module](image-processor-native.md) | Which Rust crates / N-API exports / defensive limits ship inside `image-processor.node`, and how is it wired into the `sharp()`-shaped JS facade? |
| 12 | [Operations and native-support architecture](architecture.md) | How do debug, telemetry, gateway, updater, doctor, and native helpers sit around the runtime without owning the local agent loop? |
...
- Remote-runner HTTPS egress, synchronized files, `/uploads`, and staged MCP lanes are documented in [Remote-environment egress and file staging](../04-sessions-persistence-remote/remote-environment-egress-and-file-staging.md).
